...
export default function ProjectRecruitImage() {
  ...
  const onPick = (e) => {
    ...
    if (file.size > 10 * 1024 * 1024) return alert('10MB 이하 이미지만 업로드할 수 있어요.');
  ...
  const removeImage = () => { setImageDataUrl(null); closeSheet(); };
    ...
    <div className="page recruit-image-page">
      ...
      <div className="container">
        ...
        <button type="button" className="cover-card" onClick={openSheet} aria-label="대표 이미지 업로드">
          {imageDataUrl ? <img src={imageDataUrl} alt="대표 이미지 미리보기" /> : <span className="plus" aria-hidden="true" />}
        </button>
      ...
      {sheetOpen && createPortal(
        <>
          ...
          <div className="recruit-action-sheet" role="dialog" aria-modal="true">
